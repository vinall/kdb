...
importfile:{[f] if[() ~ key hsym `$f; .log.info f," path not present";:()]; \l f; };
...
.service.sub:{
  .log.info "client connected on handle ", (string .z.w);
...
.service.upd:{
    //.log.info "update on table ", (string x);
    //.log.info "updates ";
    .log.info
    $[ (99h == type y) or (98h == type y); .service.pub[x;update tp_time:.z.P from y]; .service.pub[x;] each flip .z.P,y];
   };
